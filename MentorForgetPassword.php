...
if(isset($_REQUEST["ForgetPassword"])){
	...
	//echo $OTP;
	$search="SELECT * FROM MentorAuth where email='$EMAIL'";
	$result=mysqli_query($conn,$search);
	$num_rows = mysqli_num_rows($result);
	if($num_rows){
		//Upadate otp in database
		$InsertOTP = "UPDATE MentorAuth SET otp='$OTP' WHERE email='$EMAIL'";
  ...
  $email = new \SendGrid\Mail\Mail(); 
		...
		$email->addContent(
    "text/html", 
"<a href='http://13.126.165.2/Mentorreset.php?EMAIL=".$EMAIL."&OTP=".$OTP."'>Click To Reset password</a><br>
			<h2>Your OTP : ".$OTP."</h2>"
);
...
if ($sendgrid->send($email)) {
 $message = "Please Check Your email for OTP or Reset Link!";
    ...
    $_SESSION["EMAIL"]=$EMAIL;
     echo "<script>
  window.location.href='Mentorreset.php';
  </script>";
}
