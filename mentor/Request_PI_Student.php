...
$sql = "SELECT * FROM `PersonalInterview` WHERE `mentor_email`='$mentor_email' AND `schedule`=false";
...
if($result) {
    ...
    if($result->num_rows == 0) {
        echo "<h1 class='text-center' style='color:red;'>There are no request for Personal Interview as of now!!!!</h1>";
    }
    else {
...
$Data=mysqli_fetch_array($StudentDataResult);
            ...
            $GiveFeedback="<a href='PI_Schedule_Allocation.php?id={$row['id']}'><i class='fa fa-clock-o fa-2x' aria-hidden='true' style='color:pink;'></i></i></a>";
