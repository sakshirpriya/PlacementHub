...
if($_GET['EMAIL'] && $_GET['OTP'])
{
  $email=$_GET['EMAIL'];
  $otp=$_GET['OTP'];
  $select="SELECT email,otp from MentorAuth where email='$email' AND otp='$otp'";
  $result=mysqli_query($conn,$select);
  $num_rows = mysqli_num_rows($result);
  if($num_rows)
  {$_SESSION["email"]=$email;
    echo "<script>
  window.location.href='MentorChangePassword.php';
  </script>";
  }else{
  ...
  if($password1 == NULL || $password2 == NULL || $otp== NULL){
    ...
    echo "<script type='text/javascript'>alert('$message');</script>";
     ...
     if($password2!==$password1){
      ...
      $Search="SELECT otp FROM MentorAuth WHERE email='$email'";
